...
static BOOT_SELECTED: core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(false);
static SELECTED_ENTRY: core::sync::atomic::AtomicUsize = core::sync::atomic::AtomicUsize::new(0);
static BOOT_CANCELLED: core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(false);
...
impl slint::platform::Platform for Platform {
    ...
    fn run_event_loop(&self) -> Result<(), slint::PlatformError> {
        ...
        loop {
            ...
            // Check if boot was selected or cancelled
            if BOOT_SELECTED.load(Ordering::Relaxed) {
                return Ok(());
            }
            if BOOT_CANCELLED.load(Ordering::Relaxed) {
                return Ok(());
            }
            ...
            {
                position.x +=
                    (mouse.relative_movement[0] as f32) * 7.0 / (mouse_mode.resolution[0] as f32);
                position.y +=
                    (mouse.relative_movement[1] as f32) * 7.0 / (mouse_mode.resolution[1] as f32);
...
fn main() -> Status {
    ...
    // Read boot entries
    let entries = match read_loader_entries() {
        Ok(e) => e,
        Err(i) => {
            uefi::println!("Failed to read boot entries\n{}", i);
            return Status::LOAD_ERROR;
        }
    };

    if entries.is_empty() {
        uefi::println!("No boot entries found");
    ...
    // Convert boot entries to Slint model
    let slint_entries = slint::VecModel::default();
    for entry in &entries {
        slint_entries.push(BootEntryData {
            title: entry.title.clone().into(),
            version: entry.version.clone().unwrap_or_default().into(),
            options: entry.options.clone().unwrap_or_default().into(),
            has_version: entry.version.is_some(),
            has_options: entry.options.is_some(),
        });
    }
    ui.set_boot_entries(Rc::new(slint_entries).into());

    // Handle boot selection
    let entries_clone = entries.clone();
    ui.on_boot_selected(move |index| {
        SELECTED_ENTRY.store(index as usize, Ordering::Relaxed);
        BOOT_SELECTED.store(true, Ordering::Relaxed);
    });

    ui.on_cancel_boot(move || {
        BOOT_CANCELLED.store(true, Ordering::Relaxed);
    });

    // Run UI
    ui.run().unwrap();

    // Handle boot selection result
    if BOOT_CANCELLED.load(Ordering::Relaxed) {
        uefi::println!("Boot cancelled by user");
        return Status::ABORTED;
    }

    if BOOT_SELECTED.load(Ordering::Relaxed) {
        let selected_idx = SELECTED_ENTRY.load(Ordering::Relaxed);
        if selected_idx < entries.len() {
            ...
            if let Some(path_linux) = &entry.linux {
                match load_efi_from_path(path_linux, entry.initrd.as_deref(), entry.options.as_deref()) {
                    Ok(_) => {},
                    Err(_) => {
                        uefi::println!("Failed to load kernel");
                        return Status::LOAD_ERROR;
                    }
                }
            } else if let Some(path_efi) = &entry.efi {
                match load_efi_from_path(path_efi, None, entry.options.as_deref()) {
                    Ok(_) => {},
                    Err(_) => {
                        uefi::println!("Failed to load EFI application");
                        return Status::LOAD_ERROR;
                    }
                }
            }
